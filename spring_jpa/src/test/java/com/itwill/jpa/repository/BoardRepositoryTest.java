...
class BoardRepositoryTest extends SpringJpaApplicationTests{
	...
	void board_select_page() {
		...
		Pageable pageable=PageRequest.of(pageNumber, pageSize,Sort.by("groupno").descending().and(Sort.by("step").ascending()));
		/*
		...
		- Page 객체는 주로 데이터베이스 쿼리 결과의 한 페이지를 나타내며, 
		  페이징된 데이터를 관리하고 검색 결과를 페이지별로 표시하는 데 사용됩니다.
