			...
			<ul>
				<c:forEach items="${guestList}"  var="guest" varStatus="status" >
					...
					<c:if test="${status.count%2==0}">
						<li>${guest.guest_no},${guest.guest_name}--> ${status.count}</li>
					</c:if>
				</c:forEach>
			</ul>
		</li>
		<li>---------------Map--------------
			<ul>
				<c:forEach items="${guestMap}" var="entry">
					<code>
						&lt;c:set var="guest" value="&dollar;{entry.value}" scope="page"/&gt;
						pageContext.setAttribute("guest",entry.value);
					</code>
					<c:set var="guest" value="${entry.value}" scope="page"/>
					<li>${guest.guest_no}:${guest.guest_name }</li>
				</c:forEach>
			</ul>
		</li>
		
		<c:set  var="tempVar" scope="page" value="123" />
		<%
			pageContext.setAttribute("tempVar", 123);
		%>
		<li>${tempVar}</li>
	</ul>
</body>
